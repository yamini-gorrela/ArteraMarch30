...
import paymentRecord from '@salesforce/apex/paymentAllocationController.getRecordDetails';
import createAppliedPayment from '@salesforce/apex/paymentAllocationController.createAppliedPayment';
import updateClaimlines from '@salesforce/apex/paymentAllocationController.updateClaimlines';
//import getClaimLinesData from '@salesforce/apex/paymentAllocationController.getClaimLinesData';

// import CreateJournalEntry from '@salesforce/apex/paymentAllocationController.CreateJournalEntry';
...
export default class PaymentAllocationLwc extends LightningElement {
...
if(payerTypeName == 'Self Pay'){
   ...
   handlechangedBalance(event) {
    const index = event.target.name;
    const value = parseFloat(event.target.value);
    
    if (!isNaN(value)) {
        this.claimLinesList[index].Balance = value;
    }
}
   ...
   handleCancel(){
    ...
    if (this.totalclaimAmount > this.unalloacte) {
        ...
        let claimLineRecord = [];
        for (let i = 0; i < this.claimLinesList.length; i++) {
        ...
        if (claimLineRecord.length) {
            createAppliedPayment({
                listofRecords: JSON.stringify(claimLineRecord),
                paymentId: this.recordId,
                categoryType: this.selectedValue,
                glaccount: this.glAccountId,
                additionalAmount: this.additionalAmount
            })
            .then(response => {
                if (response && response.isSuccess) {
                    return updateClaimlines({
                        lstSelectedClaimLines: this.lstSelectedClaimLines,
                        updatePaymentList: this.updatePaymentlist
                    }).then(() => {
        ...
        if(this.payertype != null && this.checkPayerType == 'Payer'){
            ...
            for(var i=0; i<this.claimLinesList.length;i++){
                if(this.claimLinesList[i]['IsChecked'] == true && this.claimLinesList[i]['Amount'] != null && this.claimLinesList[i]['Amount'] != ''){
                    ...
                    console.log('this.claimLinesList--'>this.claimLinesList[i]['Balance']);
           ...
           if(parseFloat(totalclaimAmount).toFixed(2) > this.unalloacte){
    ...
    handleSelect2(rowIndex, ClaimRecords){ 
        ...
        for(var i=0; i<this.claimLinesList.length;i++){
            ...
            if(this.claimLinesList[i]['IsChecked'] == true && this.claimLinesList[i]['Amount'] != null && this.claimLinesList[i]['Amount'] != ''){
                this.claimAmount = parseFloat(parseFloat(this.claimLinesList[i]['Amount']).toFixed(2));
                console.log('this.claimAmount --->'+this.claimAmount);
                totalclaimAmount += this.claimAmount;
        ...
        if(parseFloat(totalclaimAmount).toFixed(2) > this.unalloacte){
            ...
            console.log('warningmsg---->'+totalclaimAmount);
            console.log('warningmsgallocate---->'+this.unalloacte);
            this.openWarningMsg = true;
        }
    }   
    ...
    if(ClaimRecords[rowIndex]['Amount'] != null && ClaimRecords[rowIndex]['Amount'] != '' && ClaimRecords[rowIndex]['AllowedAmount'] != null && ClaimRecords[rowIndex]['AllowedAmount'] != ''){
        ...
        ClaimRecords[rowIndex]['Balance'] = (ClaimRecords[rowIndex]['TotalAmount']-ClaimRecords[rowIndex]['Payments'] - ClaimRecords[rowIndex]['Amount']).toFixed(2);
        console.log('claimRecordsTotalAmount------>',ClaimRecords[rowIndex]['TotalAmount'] );
        console.log('claimRecordsPayments------>',ClaimRecords[rowIndex]['Payments'] )
        console.log('claimRecordsAmount------>',ClaimRecords[rowIndex]['Amount'] )
        negativeBalance = ClaimRecords[rowIndex]['Balance'];
    ...
    //   handleOverBalance(event){
    //     var rowIndex = Number(event.target.name);
    //     var newClaimRecords =[];
    //     newClaimRecords = JSON.parse(JSON.stringify(this.claimLinesList));
    //     newClaimRecords[rowIndex]['OverriddenBalance'] = event.target.value;
    //         this.claimLinesList = newClaimRecords;
    //   }
